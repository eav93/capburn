...
| `--arch`       | `auto`         | `fixed`, `fixed-global`, `fixed-seq`, `fixed-seq-pool`, `ctc`, or `auto`. |
| `--preprocess` | `auto`         | `stretch` (exact resize) or `fit` (preserve aspect + pad).     |
| `--input-size` | `auto`         | Model input size: dominant dataset size or `WIDTHxHEIGHT`.      |
| `--backend`    | `wgpu`         | `wgpu` (GPU), `cpu`, or `cuda`.                                 |
| `--epochs`     | `30`           | Number of training epochs.                                     |
| `--batch-size` | `64`           | Batch size.                                                    |
| `--lr`         | `0.0005`       | Adam learning rate.                                            |
| `--dropout`    | `0.2`          | Dropout probability in the recognition head.                   |
| `--augment`    | `auto`         | `light`, `medium`, `strong`, or `off`.                         |
| `--no-augment` | off            | Disable training-image augmentation (affine + photometric).    |

### Architectures (`--arch`)

All architectures share one CNN backbone. `--input-size auto` uses the dominant
dataset image size, while an explicit value like `128x32` forces that grayscale
model input size. The sequence variants use temporal convolutions over the CNN
width axis instead of RNN layers, so training and CPU inference stay fast.
...
| `fixed-global` | Whole-width classifier + cross-entropy | Fixed-length captchas whose characters do not align with equal-width slots. |
| `fixed-seq` | Temporal residual convs + whole-width classifier + cross-entropy | CRNN-like fixed-length captchas where global context helps, without the runtime cost of LSTM. |
| `fixed-seq-pool` | Temporal residual convs + positional slots + cross-entropy | Fixed-length captchas where characters mostly keep slot order, but local sequence smoothing helps. |
...
### Comparing fixed-length architectures

For a new fixed-length dataset, keep preprocessing, augmentation, dropout,
input size, learning rate, seed and split unchanged while comparing `--arch`
variants. Start with the current strong baseline:

```bash
./target/release/capburn train --data ./data --dataset digits5 \
  --arch fixed-global --preprocess fit --augment medium --dropout 0.3 \
  --epochs 100 --out ./artifacts/digits5-fixed-global-fit-drop03
```

Then compare the CRNN-like temporal variants with the same hyperparameters:

```bash
./target/release/capburn train --data ./data --dataset digits5 \
  --arch fixed-seq --preprocess fit --augment medium --dropout 0.3 \
  --epochs 100 --out ./artifacts/digits5-fixed-seq-fit-drop03

./target/release/capburn train --data ./data --dataset digits5 \
  --arch fixed-seq --preprocess fit --augment medium --dropout 0.4 \
  --epochs 100 --out ./artifacts/digits5-fixed-seq-fit-drop04

./target/release/capburn train --data ./data --dataset digits5 \
  --arch fixed-seq-pool --preprocess fit --augment medium --dropout 0.3 \
  --epochs 100 --out ./artifacts/digits5-fixed-seq-pool-fit-drop03
```

The fair metric is `Best validation accuracy` printed during training.
`capburn eval` reports full-dataset accuracy (useful for error analysis) and,
when the artifacts folder contains `config.json`, also reconstructs the
training validation split and reports held-out accuracy separately.
...
```bash
./target/release/capburn eval \
  --data ./data \
  --dataset digits5 \
  --artifacts ./artifacts/digits5-fixed-global-fit-drop03
```

The command reports full-captcha accuracy, character accuracy, prediction
lengths, per-position accuracy and top confusions. When the artifacts folder
contains `config.json`, it also reports held-out accuracy on the reconstructed
...
```
model.json    # charset + captcha length + architecture + preprocessing + input size
...
| `model.json`       | Charset, length, architecture, preprocessing and input size — read on load. |
...
| `--arch`       | `auto`         | `fixed`, `fixed-global`, `fixed-seq`, `fixed-seq-pool`, `ctc` или `auto`. |
| `--preprocess` | `auto`         | `stretch` (точный resize) или `fit` (сохранить aspect + padding). |
| `--input-size` | `auto`         | Размер входа модели: dominant размер датасета или `WIDTHxHEIGHT`. |
...
| `--dropout`    | `0.2`          | Вероятность dropout в recognition head.                         |
| `--augment`    | `auto`         | `light`, `medium`, `strong` или `off`.                          |
| `--no-augment` | выкл           | Отключить аугментацию тренировочных изображений.               |

### Архитектуры (`--arch`)

Все архитектуры используют общий CNN-backbone. `--input-size auto` берёт
dominant размер картинок в датасете, а явное значение вроде `128x32` фиксирует
размер grayscale входа модели. Sequence-варианты используют temporal
convolutions по ширине CNN вместо RNN, поэтому обучение и CPU-инференс остаются
быстрыми.
...
| `fixed-global` | Классификатор всей ширины + cross-entropy | Фиксированная длина, но символы не попадают в равные слоты. |
| `fixed-seq` | Temporal residual convs + классификатор всей ширины + cross-entropy | CRNN-like fixed-length капчи, где нужен глобальный контекст, но без цены LSTM. |
| `fixed-seq-pool` | Temporal residual convs + позиционные слоты + cross-entropy | Фиксированная длина, где порядок символов близок к слотам, но помогает локальное sequence-сглаживание. |
...
контраст, шум) для лучшей генерализации на маленьких датасетах; силу задает
`--augment`, отключение — `--augment off` или `--no-augment`. Каждая эпоха
печатает loss, per-char и full-captcha точность на
отложенной выборке; на диск сохраняется только лучшая эпоха.

### Сравнение fixed-length архитектур

Для нового fixed-length датасета сравнивайте `--arch` варианты при одинаковых
preprocessing, input size, augmentation, dropout, learning rate, seed и split.
Начните с текущего сильного baseline:
...
Потом сравните CRNN-like temporal варианты с теми же гиперпараметрами:
...
Честная метрика — `Best validation accuracy`, которую печатает обучение.
`capburn eval` печатает точность по всему датасету (полезно для разбора ошибок),
а при наличии `config.json` в папке артефактов дополнительно восстанавливает
валидационный сплит обучения и отдельно печатает held-out accuracy.
...
# 4. Следить за прогрессом (loss + точность per-char / full-captcha по эпохам)
tail -f train.log
...
Проверить сохранённую модель на размеченном датасете:
...
Команда печатает full-captcha accuracy, character accuracy, длины предсказаний,
точность по позициям и top ошибок. Если в папке артефактов есть `config.json`,
дополнительно печатается held-out accuracy на восстановленном валидационном
сплите обучения.
...
```
model.json    # алфавит + длина капчи + архитектура + preprocessing + input size
...
| `model.json`       | Алфавит, длина, архитектура, preprocessing и input size — читаются при загрузке. |
