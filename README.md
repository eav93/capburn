...
| `--arch`       | `auto`         | `fixed`, `fixed-global`, `ctc`, or `auto`.                 |
...
| `model.json`       | Charset, length, architecture and preprocessing — read on load. |
...
| `--arch`       | `auto`         | `fixed`, `fixed-global`, `ctc` или `auto`.                   |
